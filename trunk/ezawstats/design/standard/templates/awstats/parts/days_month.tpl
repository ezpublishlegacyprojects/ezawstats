...
{foreach $awstats.data.day as $day_info}
    <td class="cdata images"><img src={'vv.png'|ezimage} alt="{'Numbers of visits:'|i18n( 'awstats/stats' )} {$day_info.Visits}" height="{$day_info.Visits|mul( 150 )|div( $awstats.data.general.TotalVisits )}" width="4" /><img src={'vp.png'|ezimage} alt="{'Pages:'|i18n( 'awstats/stats' )} {$day_info.Pages}" height="{$day_info.Pages|mul( 150 )|div( $awstats.data.total.Pages|sum( $awstats.data.total.NotViewedPages ) )}" width="4" /><img src={'vh.png'|ezimage} alt="{'Hits:'|i18n( 'awstats/stats' )} {$day_info.Hits}" height="{$day_info.Hits|mul( 150 )|div( $awstats.data.total.Hits|sum( $awstats.data.total.NotViewedHits ) )}" width="4" /><img src={'vk.png'|ezimage} alt="{'Bandwidth:'|i18n( 'awstats/stats' )} {$day_info.Bandwidth|si( 'byte' )}" height="{$day_info.Bandwidth|mul( 150 )|div( $awstats.data.total.Bandwidth|sum( $awstats.data.total.NotViewedBandwidth ) )}" width="4" /></td>
{/foreach}
</tr>
<tr class="day_line">
{foreach $awstats.data.day as $day_info}
